...
public class ZXingCode {
    ...
    public static void main(String[] args) throws WriterException
    {
        try
        {
            getLogoQRCode("http://wap.hn.10086.cn/shop/simBuy/simH5OnlineToApply?confId=2002&recmdCode=NjM4NjMyNTQ4MDMwOTk2NDg=", "移动大王卡");
        }
        catch (Exception e)
        {
            e.printStackTrace();
...
//      String filePath = (javax.servlet.http.HttpServletRequest)request.getSession().getServletContext().getRealPath("/") + "resources/images/logoImages/llhlogo.png";
        ...
        String filePath = "/Users/cc/Desktop/WechatIMG14.jpeg";  //TODO
    ...
    public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName)
    {
        try
        {
            ...
            BufferedImage outImage1 = new BufferedImage(400, 600, BufferedImage.TYPE_4BYTE_ABGR);
            Graphics2D outg1 = outImage1.createGraphics();
            //画二维码到新的面板
            outg1.setColor(Color.BLACK);
            System.out.println(widthLogo);
            outg1.setFont(new Font(Font.DIALOG_INPUT,Font.PLAIN,20)); //字体、字型、字号
            outg1.drawImage(logo, 0, 0, widthLogo,widthLogo, null);
            int strWidth3 = outg1.getFontMetrics().stringWidth("谢江琼推荐你办理");
            outg1.drawString("谢江琼推荐你办理", 200  - strWidth3/2, 40);
            outg1.drawImage(image, 0, 200, image.getWidth(), image.getHeight(), null);

            //把商品名称添加上去，商品名称不要太长哦，这里最多支持两行。太长就会自动截取啦
            if (productName != null && !productName.equals("")) {
                //新的图片，把带logo的二维码下面加上文字
                BufferedImage outImage = new BufferedImage(400, 700, BufferedImage.TYPE_4BYTE_ABGR);
                Graphics2D outg = outImage.createGraphics();
                //画二维码到新的面板
                outg.drawImage(outImage1, 0, 0, image.getWidth(), image.getHeight(), null);
                //画文字到新的面板
                outg.setColor(Color.BLACK);
                outg.setFont(new Font("SansSerif",Font.PLAIN,30)); //字体、字型、字号
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
                    ...
                    outg2.setFont(new Font(Font.SANS_SERIF,Font.PLAIN,30)); //字体、字型、字号
            ...
            ImageIO.write(image, "png", new File("/Users/cc/Desktop/TDC-" + new Date().getTime() + "test.png")); //TODO
