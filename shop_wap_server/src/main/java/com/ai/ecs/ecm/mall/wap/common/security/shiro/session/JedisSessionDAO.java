 ...
 */
package com.ai.ecs.ecm.mall.wap.common.security.shiro.session;

import com.ai.ecs.ecm.mall.wap.common.Servlets;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void touch(Serializable sessionId, long timeout) {
		// 设置超期时间
		int timeoutSeconds = (int) (timeout / 1000);
		jedisCluster.expire((sessionKeyPrefix + sessionId),timeoutSeconds);
	}
	...
	public void update(Session session) throws UnknownSessionException {
       ...
       try {
		   ...
		   jedisCluster.hset(sessionKeyPrefix, session.getId().toString(),
				   principalId + "|" + session.getTimeout() + "|"
						   + session.getLastAccessTime().getTime());*/
		   jedisCluster.set(JedisClusterUtils.getBytesKey(sessionKeyPrefix + session.getId()),
				   JedisClusterUtils.toBytes(session));

		   // 设置超期时间
		   int timeoutSeconds = (int) (session.getTimeout() / 1000);
		   jedisCluster.expire((sessionKeyPrefix + session.getId()),
				   timeoutSeconds);
		   logger.debug("update {} {}", session.getId(),
				   request != null ? request.getRequestURI() : "");
	   }catch (Exception e){
		   logger.error("update redis session error:",e);
	   }
	}
...
//		jedisCluster.hdel(sessionKeyPrefix, session.getId().toString());
		jedisCluster.del(sessionKeyPrefix + session.getId());
...
//
//		try {
//			Map<String, String> map = jedisCluster.hgetAll(sessionKeyPrefix);
...
//						// SESSION验证失败
//						catch (Exception e2) {
//							jedisCluster.hdel(sessionKeyPrefix, e.getKey());
//						}
//					}
//					// 存储的SESSION不符合规则
//					else {
...
//		return sessions;
	...
	protected Session doReadSession(Serializable sessionId) {
		...
		try {
			...
			session = (Session) JedisClusterUtils.toObject(jedisCluster
					.get(JedisClusterUtils.getBytesKey(sessionKeyPrefix + sessionId)));
			// }
			...
			logger.error("Redis seesion 获取异常：",e);
		}
		return session;
	}
