...
function share() {
    try{
        if(window.Webtrends){
            Webtrends.multiTrack({
                argsa: ["WT.event","WZXD_FX"],
                delayTime: 100
            })
        }else{
            if(typeof(dcsPageTrack)=="function"){
                dcsPageTrack ("WT.event","WZXD_FX",true);
            }
        }
    ...
    if(is_weixn()){
        ...
        if(uAgent.indexOf("Hebao")>-1 || uAgent.indexOf("hebao")>-1){
            ...
            shareHeBao("我推荐你办理"+$("#cardTypeName").val(),$("#recmdUserLink").val(),"我推荐你办理"+$("#cardTypeName").val()+str)
        }else{
...
function wxReady() {
    ...
    cardTypeName = "我推荐你办理" + $("#cardTypeName").val();

    var desc = "";
    var timelineTitle = cardTypeName;

    if ($("#cardTypeName").val() == '移动大王卡') {
        desc = "("+$("#planDesc").val()+")";
        img = "http://wap.hn.10086.cn/shop/static/images/share-2.jpg";
        timelineTitle += desc;
    } else {
        if($("#planDesc").val()!=''){
            desc = "("+$("#planDesc").val()+")";
