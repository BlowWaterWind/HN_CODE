			...
			function(data) {
				...
				if (data) {
					for(var i = 0; i < data.list.length; i++) {
						...
						var str1 = "<ul style='display:block;' class='cur-ul'>" +
					    "<li class='cur-li'>" +
						...
						if (orderView.detailList) {
							...
							for (var j = 0; j < orderView.detailList.length; j++) {
								...
								for (var key in orderView.actionMap) {
									var s4="";
								<!--判断是否是85商品 -->
									if(orderView.shopId!=100000002099 && orderView.orderTypeId!=undefined && orderView.orderTypeId!=6 && orderView.orderTypeId!=8){
										s4 = "<p>" +
								...
								var s2 = "";
								if (orderView.orderStatusId == 8 || orderView.orderStatusId == 12) {
									if(orderView.shopId!=100000002099) {
										s2 = "<p><a href='" + ctx + "myOrder/process?action=aftersale&act=1&orderSubId=" + orderView.orderSubId + "&subOrderDetailId=" + order.orderSubDetailId + "'  class='pull-right btn btn-bd btn-sm mr10' >申请售后</a></p>";
									}
								}
								
								var s3 = "";
								if (orderView.orderStatusId == 18) {
									s3 = "已取消";
								}
								str += (s + s1 + s2 + s3);
						...
						str1 += "<span class='sm-date pull-right font-gray'>"+ orderTime.format("yyyy-MM-dd hh:mm:ss") +"</span>" +
