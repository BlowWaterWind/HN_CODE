	...
	function ajaxHomeGoods(reflesh){
		var url = "ajaxShopList";
		var pageNo = $("#pageNo").val();
		var pageSize = $("#pageSize").val();
		var sortType = $('#sortType').val();
	    var shopName = $('#serachKey').val();
		...
		if(sortClounm!=''&&sortType!=''){
			param = '{"pageNo":'+pageNo+',"pageSize":'+pageSize+',"sortType":"'+sortType+'","sortClounm":"'+sortClounm+'","shopName":"'+shopName+'"}';
		}else{
			param = '{"pageNo":'+pageNo+',"pageSize":'+pageSize+',"shopName":"'+shopName+'"}';
		}
		...
		$.post(url,param,function(data){
			setTimeout(function() {
				...
				for(var i=0;i<data.list.length;i++){
					var d = data.list[i];
					var $clone = $('div.shopDiv:last').clone();
					$('a',$clone).attr('href',baseProject+'shops/'+d.wapShopUrl);
					$('img',$clone).attr('src',imageServerPath+d.shopLogo);
					$('.shopName',$clone).html(d.shopName);
					$('.shopAddress',$clone).html(d.shopPhysicallProvince+d.shopPhysicallCity+d.shopPhysicallCounty+d.shopPhysicallAddress);
					$('.colllection',$clone).html(d.collectionNum+'人');
