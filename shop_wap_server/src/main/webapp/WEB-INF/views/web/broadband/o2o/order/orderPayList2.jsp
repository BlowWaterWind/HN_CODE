...
<head>
    ...
    <meta name="WT.si_n" content="WDDD_ZFDD" />
...
<script type="text/html" id="tempOrderList">
    ...
    <div class="list order-list">
        <div class="order-list_hd">
            ...
            <div class="right"><span class="txt-red">
                  {{ if o2oTempOrder.order_status=='1' && o2oTempOrder.orderSub.orderStatusId == '12'}}
                        已归档
                    {{else if o2oTempOrder.order_status=='1' && (o2oTempOrder.orderSub.orderStatusId =='0' || o2oTempOrder.orderSub.orderStatusId =='1' )}}
                        待支付
                    {{else  if o2oTempOrder.order_status=='5'}}
                        同步失败
                    {{/if}}
            </span></div>
        </div>
        ...
        <div class="btn-wrap Grid -right">
            {{if o2oTempOrder.order_status=='1' && o2oTempOrder.orderSub.orderSubPayStatus =='0' && o2oTempOrder.orderSub.orderStatusId !='12'}}
            <button href="javascript:void(0)" class="btn btn-default" style="background-color: #fff;" onclick="orderPay('{{o2oTempOrder.order_temp_id}}')">支付</button>
            {{else if o2oTempOrder.order_status=='5'}}
            <button href="javascript:void(0)" class="btn btn-default" style="background-color: #fff;" onclick="resetOrder('{{o2oTempOrder.order_temp_id}}')">再次BOSS同步</button>
            {{ else if o2oTempOrder.order_status=='1' &&o2oTempOrder.orderSub.orderSubPayStatus =='1'}}
            <button class="btn btn-default" style="background-color: #fff;" onclick="window.location.href='${ctx}/o2oOrder/payDetail?orderId={{o2oTempOrder.orderSub.orderId}}'">支付明细</button>
            {{/if}}
        </div>
    </div>
    {{/each}}
