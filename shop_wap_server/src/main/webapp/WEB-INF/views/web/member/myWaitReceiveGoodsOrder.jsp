...
<div id="outer" class="container">
  ...
  <li id="waitReceive" class="current"><span class="text">待收货<span class="badge">${orderCount.waitReceiveCount}</span></span></li>
    ...
    %>
      <ul style="display:block;" class="cur-ul">
        <li class="cur-li">
          ...
          <p><a class="pull-right btn btn-bd btn-sm mr10" href="<%=request.getContextPath()%>/myOrder/process?action=${map.value.id}&act=${map.value.act}&orderStatusId=<%=orderView.getOrderStatusId()%>&orderSubNo=<%=orderView.getOrderSubNo()%>&orderSubId=<%=orderView.getOrderSubId()%>&subOrderDetailId=<%=subDetail.getOrderSubDetailId()%>&orderTypeId=<%=orderView.getOrderTypeId()%>" target="_self">${map.value.name}</a></p>
          <%}else if(orderView.getOrderTypeId()!=7){%>
            <c:if test="${map.value.name=='支付'}">
                <a class="pull-right btn btn-bd btn-sm mr10" href="http://www.10085.cn/web85/page/zyzxpay/wap_order.html?orderId=<%=orderView.getThirdPartyId()%>" target="_self">支付</a>
            </c:if>
          <%}%>
          </c:forEach>
          <%
          ...
          %>
        <% if(orderView.getShopId()==100000002099l&&orderView.getOrderTypeId()!=7){ %>
            <a class="btn btn-sm btn-bd pull-right logisticBtn" href="<%=request.getContextPath()%>/myOrder/to10085LogisticDetail?orderId=<%=orderView.getOrderSubId()%>&dsOrderId=<%=orderView.getThirdPartyId()%>" >查看物流</a>
        <%}else if(orderView.getOrderTypeId()!=7){%>
         <button class="btn btn-sm btn-bd pull-right logisticBtn" data-index="<%=orderView.getOrderSubId() %>"  data-logisticsNum="<%=orderView.getLogisticsNum() %>" >查看物流</button>
       <%}%>
