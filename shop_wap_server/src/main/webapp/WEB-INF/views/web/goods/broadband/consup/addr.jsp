        ...
        <div class="container active-fix">
        	<c:choose>
        		<c:when test="${minCost == '0'}">
        			<a href="${ctx}/consupostn/familynet?hasAddress=Y" class="btn btn-rose">使用该地址</a>
        		</c:when>
        		<c:when test="${minCost == '48'}">
        			<a href="${ctx}/consupostn/init?busiType=1&minCost=48&hasAddress=Y" class="btn btn-rose">使用该地址</a>
        		</c:when>
        		<c:when test="${minCost == '158'}">
        			<a href="${ctx}/consupostn/init?busiType=2&minCost=158&hasAddress=Y" class="btn btn-rose">使用该地址</a>
        		</c:when>
                <c:when test="${minCost == '88'}">
                    <a href="${ctx}/consupostn/init?busiType=1&minCost=88&hasAddress=Y" class="btn btn-rose">使用该地址</a>
                </c:when>
                <c:when test="${minCost == '98'}">
                    <a href="${ctx}/consupostn/init?busiType=2&minCost=98&hasAddress=Y" class="btn btn-rose">使用该地址</a>
                </c:when>
        		<c:otherwise>
        			<a href="${ctx}/consupostn/familynet?hasAddress=Y" class="btn btn-rose">使用该地址</a>
        		</c:otherwise>
