...
$(function() {
	...
	$("#queryAddress").bind("keyup",function(){
		...
		$.getJSON(ctx+'bandResourceQuery/qryAddressCommunityName', {
				...
				if (e.respDesc == "OK!") //接口请求成功
				{
					...
					if (typeof(dcsPageTrack)=="function"){ dcsPageTrack ("WT.si_n","宽带新装",1, "WT.si_x","选择地址",1);}
					$('#queryCommit').attr("disabled","disabled");
					$('#queryCommit').attr("class","btn btn-gray");
				} else {
	...
	$("li[name='li_broadbandItem']").bind("click",function(){
		...
		var priceDesc = $(this).find("p").html();
        ...
        if (typeof(dcsPageTrack)=="function"){ dcsPageTrack ("WT.si_n","宽带新装",1, "WT.si_x","选择产品档次"+chooseBandWidth+"M "+priceDesc,1);}
	});
	...
	$("#a_confirmInstall").bind("click",function(){
		if (typeof(dcsPageTrack)=="function"){dcsPageTrack("WT.si_n","宽带新装",0,"WT.si_x","宽带新装办理",0)}
...
function searchBuilding2(obj){
	...
	$.getJSON(ctx+'bandResourceQuery/qryAddressBuildingName', {
			...
			if (e.respDesc == "OK!") //接口请求成功
			{
				...
				$.each(ADDRESS_INFO, function (i, item) {
					$(".adress-list").append("<li><label><span class='adr-xz'><input type='radio' name='radio_address'></span><span coverType=" + item.COVER_TYPE + " address-path-building=" + item.ADDRESS_PATH + " class='adr-text'  onclick=chooseBuilding2(this)>" + item.BUILDING_NAME +  "</span></label></li>");
				});
			}
		}
	);
...
function chooseBuilding2(obj){
	...
	$.getJSON(ctx+'bandResourceQuery/qryAddressName', {
			...
			if (e.respDesc == "OK!") //接口请求成功
			{
				...
				$.each(ADDRESS_INFO, function (i, item) {
					//console.log(item);
					$(".adress-list").append("<li><label><span class='adr-xz'><input type='radio' name='radio_address'></span><span coverType=" + item.COVER_TYPE + " last-address-building=" + item.HOUSE_CODE + " class='adr-text' onclick=chooseLastAddress2(this)>" + item.ADDRESS_NAME +  "</span></label></li>");
				});
			} else {
				alert("系统异常，请稍后再试");
			}
