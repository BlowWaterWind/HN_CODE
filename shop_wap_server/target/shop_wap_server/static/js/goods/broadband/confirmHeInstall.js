$(function() {
    ...
    function initBookTime(bookStartTime) {
        ...
        var bookTimePoint = [
            new Date(bookStartTime.toLocaleDateString() + " 12:00"),
    ...
    var bookStartTime = new Date($("#bookInstallDate").val + " "
    ...
    $("#a_confirm2").bind("click",function(){
        ...
        if(recommend=="0"){
            var isEmployeNo=/^36\d{6}$/g;
            $("#p_recommend").text("员工推荐");
            if($("#employeNo").val()=="" || $("#employeNo").val()=="请输入以36开头的8位人力资源员工编号"){
                $("#span_inputMesage2").text("推荐员工编号不能为空");
                return ;
            }
            if(!isEmployeNo.test($("#employeNo").val())){
                $("#span_inputMesage2").text("员工编号格式错误,请输入以36开头的8位人力资源员工编号");
                return ;
            }
    ...
    $("#a_submitInstallOrder1").bind("click",function(){
        ...
        var hasBroadband = $("input[name='form1_hasBroadband']").val();
        if(hasBroadband == '1'){
            var checkIdCardMsg = checkIdcard(idCard);
            if(checkIdCardMsg !="验证通过!"){
                showAlert(checkIdCardMsg);
        ...
        var param = $("#form1").serializeObject();
        showLoadPop();
        $.post(ctx + "/BroadbandTrade/submitInstallOrder",param,function(data){
            hideLoadPop();
            if(data.code=="0"){
                $("#payForm_orderSubNo").val(data.orderSub.orderSubNo);
                $("#b_orderNo").text(data.orderSub.orderSubNo);
                $("#span_price").text(data.orderSub.orderSubAmount/100);
                $("#b_goodsName").text(data.orderSubDetail.goodsName);
                $(".share-box").slideDown('fast');
                $(".more-box").addClass("on");
