...
$(function() {
	...
	$("#submitBtn").click(function() {
		...
		if($("#memberRecipientName").val()==""){
			...
			//传输时对姓名加密
			var memberRecipientName = encode64($("#memberRecipientName").val());
            $("#memberRecipientNameHidden").val(memberRecipientName);
			$("#recName").html("");
		}
		if($("#memberRecipientPhone").val()==""){
		...
		if($("#memberRecipientPhone").val().indexOf("*")==-1 &&!isMobile.test($("#memberRecipientPhone").val())){
			$("#memberRecipientPhoneDiv").addClass("input-fr");
			...
			$("#memberRecipientPhoneDiv").removeClass("input-fr");
            var memberRecipientPhone = encode64($("#memberRecipientPhone").val());
            $("#memberRecipientPhoneHidden").val(memberRecipientPhone);
			$("#memberRecipientPhoneSpan").html("");
		}
		if($("#memberRecipientCounty").val()==""){
		...
		if($("#memberRecipientAddress").val()==""){
            ...
            var memberRecipientAddress = encode64($("#memberRecipientAddress").val());
            $("#memberRecipientAddressHidden").val(memberRecipientAddress);
			$("#memberRecipientAddressSpan").html("");
		}
		$.post(submitUrl,$("#addrForm").serialize(), function(data) {
			data=data.replace("\"","").replace("\"","");
			if(data=="success"){
				showAlert("修改成功！",gotofuc);
			}else{
				showAlert(data);
