...
<div class="container pd-t45">
    <div class="qr-dl gray-bg">
        <div>
            <p>订单号：${changeOrder.orderSub.orderSubNo}</p>
            <p>下单时间：<fmt:formatDate value="${changeOrder.orderSub.orderTime}" pattern="yyyy-MM-dd HH:mm:ss"/></p>
            <p class="font-rose">订单状态：${changeOrder.orderSub.orderStatus.orderStatusName}</p>
        </div>
    ...
    <div class="cur-ul cur-con">
        <div class="cur-li cur-bd">
            ...
            <div class="tabcon-cl tab-pic">
                <!-- <a href="#"> -->
                <dl>
                    <dt><img src="${tfsUrl}${changeOrder.orderSub.detailList[0].goodsImgUrl}" /></dt>
                    <dd>
                        <h2>商品名称：${changeOrder.orderSub.detailList[0].goodsName}</h2>
                        <span class="tab-zh">${changeOrder.orderSub.detailList[0].goodsFormat}</span></dd>
                    <i class="dy">
                        <p class="tabcon-cl tabcon-cl03">${changeOrder.orderSub.detailList[0].goodsSkuPrice/100}</p>
                        <p class="tabcon-cl tabcon-cl03 tab-lr">×${changeOrder.orderSub.detailList[0].goodsSkuNum}</p>
                    </i>
                </dl>
                <!-- </a>-->
            </div>
    ...
    <div class="cur-con">
        <ul class=" box10 white-bg">
            <%--<p class="tabcon-cl"> <span class="pull-jl02">换货信息</span> </p>--%>
            <div>
                <p>退货单号：${changeOrder.changeOrderNo}</p>
                <p>申请时间：<fmt:formatDate value="${changeOrder.applyTime}" pattern="yyyy-MM-dd HH:mm:ss"/></p>
                <p>退货原因：${changeOrder.changeReson}</p>
                <c:if test="${! empty changeOrder.changeResonDesc}">
                    <p>原因描述：${changeOrder.changeResonDesc}</p>
                </c:if>
                <!-- TODO：上传的图片 -->
            </div>
        </ul>
    ...
    <div class="cur-con">
        <ul class=" box10 white-bg">
            ...
            <li>
                <c:choose>
                    <c:when test="${! empty changeOrder.proxyApplyPerson}">
                        <span class="pull-left">${changeOrder.proxyApplyPerson}：代理申请换货服务</span>
                    </c:when>
                    <c:otherwise>
                        <span class="pull-left">${changeOrder.applyLoginName}：申请换货服务</span>
                    </c:otherwise>
                </c:choose>
                <span class="pull-right"><fmt:formatDate value="${changeOrder.applyTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
            </li>

            <!-- 处理信息 -->
            <c:forEach items="${changeOrder.changeOrderActLogL}" var="l">
                <c:if test="${l.operator}">
                <li>
