        ...
        <div class="form-box mt-12">
            <ul class="form-list">
                ...
                <li>
                    <span>推荐人工号</span>
                    <div class="form-rt"><i class="clear-index" style="display: none"></i>
                    <input type="text" name="managerId" id="managerId" lass="form-input" onkeyup="staffKey(this)" placeholder="请输入推荐人工号" /></div>
                </li>
            </ul>
        </div>
        <input type="hidden" name="bankType" value="CCB">
    </form>
    <!--表单 end-->
    <!--协议 start-->
    <div class="agreement-box">
        <input type="checkbox" id="ckbox" class="checkbox" />
        <label for="ckbox">本人同意<a class="font-red" data-toggle="modal" data-target="#myModal02">《个人信息授权书》</a></label>
    </div>
    <!--协议 end-->
    <div class="gobal-btn">
        <a class="btn btn-blue w50" data-toggle="modal" id="submitBtn">资格查验</a>
...
<div class="modal fade gobal-modal" id="myModal02" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer"><a class="btn btn-blue w50" data-dismiss="modal">确定</a></div>
        </div>
    </div>
</div>
<!--协议弹框 end --><!--加载弹框 start-->
<div class="modal fade gobal-modal" id="myModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <div class="load-con">
        ...
        $.ajax({
            ...
            success: function (data1) {
                ...
                if (data1.code == '0' || data1.code == '2') {
                    ...
                    window.location.href = "${ctx}ccbank/toChooseOffer?type="+data1.data.type+"&orderSubNo="+data1.data.orderSubNo+"&phone="+data1.data.phone;
                    //名下已经有湖南移动的号码
                }else if(data1.code == '3'){
                    //去确认套餐
                    var destUrl="${ctx}ccbank/toConfirmOffer?planId="+ data1.data.planId+
                    ...
                    toggleModal('myModal',data1.message,'前往确认',destUrl);
                }else if(data1.code == '4'){
                    //去支付
                    var destUrl="${ctx}ccbank/toPay?orderSubNo="+data1.data.orderSubNo;
                    toggleModal('myModal',data1.message,'前往支付',destUrl);
                }else if(data1.code == '-2'){
                    //非湖南移动号码,通过建设银行的推荐链接办理
                ...
                try{
                    dcsPageTrack("WT.si_n", "建设银行活动办理", 0, "WT.si_x", "号码检查", 0);//插码使用
                }catch (e) {
                    console.log("插码失败")
                }
            },
