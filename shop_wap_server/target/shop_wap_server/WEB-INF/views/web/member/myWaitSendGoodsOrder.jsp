...
<div id="outer" class="container">
 <ul id="tab" class="tabList list-con container">
  ...
  <li id="waitSend" class="current"><span class="text">待发货<span class="badge">${orderCount.waitSendCount}</span></span></li>
    ...
    %>
      <ul style="display:block;" class="cur-ul">
        <li class="cur-li">
          ...
          %>
          <%if(orderView.getOrderTypeId()!=7){%>
          <a href="#" class="pull-right btn btn-bd btn-sm mr10 sendGoodsWarnBtn" data-index="<%=orderView.getOrderSubId()%>">发货提醒</a>
          <%}%>
          	 <span class="sm-date pull-right font-gray"><%=DateUtils.formatDateHMS(orderView.getOrderTime()) %></span>
            <span class="pull-left pull-gd"><strong>共<%=orderView.getDetailList()==null?0:orderView.getDetailList().size()%>件商品</strong><b><em>实付:</em>￥<%=orderView.getOrderSubPayAmount()/100.00%></b></span> </p>
        </li>
    </ul>
    <%  }}%>
  </div>
