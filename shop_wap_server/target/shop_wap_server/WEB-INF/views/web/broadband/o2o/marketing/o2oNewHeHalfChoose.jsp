...
<div class="container">
    <form action="submitTempOrder" id="form1" name="form1" method="post">
        ...
        <input type="hidden" id="productLevel" name="productLevel"/>
    ...
    <div class="hjt-list">
        ...
        <table class="hjt-table">
            <thead>
            <tr>
                <th colspan="7" class="th-blue">和家庭半年包促销</th>
            </tr>
            <tr>
                <th rowspan="2">促销</th>
                <th rowspan="2">套餐
                ...
                <th rowspan="2">产品
                    <br>内容</th>
                <th colspan="2">存费送光猫机顶盒</th>
                <th rowspan="2">初装费
                    <br>（元）</th>
                <th rowspan="2" class="font-red">套餐选择</th>
            </tr>
            <tr>
                <th>促销价
                    <br>元</th>
            ...
            <c:forEach items="${heBroadbandItemList}" var="broadbandItem">
                <tr>
                   <c:choose>
                       <c:when test="${broadbandItem.productLevel ne curentPackage}">
                           <td class="tr-disabled" title="productName">半年包</td>
                           <td class="tr-disabled" title="productLevel">${broadbandItem.productLevel}</td>
                           <td class="tr-disabled" title="productDesc">${broadbandItem.productDesc}</td>
                           <td class="tr-disabled" title="price"><fmt:formatNumber value="${broadbandItem.price/100}" type="number"/></td>
                           <td class="tr-disabled" title="device">光猫
                               <br>机顶盒</td>
                           <td class="tr-disabled" title="installPrice">0</td>
                           <td class="tr-disabled tr-white"><input type="radio" name="radio" /></td>
                       </c:when>
                       <c:otherwise>
                           <td class="tr-blue" title="productName">半年包</td>
                           <td class="tr-blue" title="productLevel">${broadbandItem.productLevel}</td>
                           <td class="tr-blue" title="productDesc">${broadbandItem.productDesc}</td>
                           <td class="tr-blue" title="price"><fmt:formatNumber value="${broadbandItem.price/100}" type="number"/></td>
                           <td class="tr-blue" title="device">光猫
                               <br>机顶盒</td>
                           <td class="tr-blue" title="installPrice">0</td>
                    ...
                    <input type="hidden" name="product_Level" value="${broadbandItem.productLevel}"/>
        ...
        <div class="hjt-balance">
            <div class="balance-info">
                <p>当前套餐档次：${curentPackage}元/月${packageName}</p>
            </div>
        </div>
        <!--当前套餐余额 end-->
    </div>
    ...
    <div class="hjt-list">
        ...
        <div class="hjt-rules">
            <div class="rules-list">
                <%--<p>1、该活动与其他存费类活动互斥，但与其他业务年包不互斥；</p>--%>
            </div>
        </div>
    </div>
    <!--业务规则 end-->
    ...
    $(document).ready(function() {
        $("tbody>tr").click(function () {
            $(this).find(":radio").attr("checked", true)
                    .end().find("td:not([title='installPrice'])").addClass("selected")
                    .end().siblings().find("td").removeClass("selected");
            if (!$(this).find("td[title='installPrice']").hasClass("tr-disabled")) {
                $(this).find("td[title='installPrice']").addClass("selected").end().siblings().find("td[title='installPrice']").removeClass("selected");
            }
            ...
            var product_Level = $(this).find("input[name='product_Level']").val();
            ...
            $("#price").val(price);
            $("#productLevel").val(product_Level);
