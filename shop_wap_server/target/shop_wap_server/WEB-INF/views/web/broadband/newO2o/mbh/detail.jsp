...
<head>
	...
	<meta name="WT.si_n" content="MBH_DKYCK" />
...
<body>
	...
	<div class="container">
		<div class="wf-list sub-info sub-tc">
	    	<div class="wf-ait clearfix">
		        ...
		        <div class="wf-con">
		          	...
		          	<p class="font-gray">
			            ...
			            <span class="sus-text font-3">${broadbandDetailInfo.newProductsInfo[0].start_date} ~ ${broadbandDetailInfo.endTime}</span>
		          	</p>
		          	<p class="font-gray">
	      	...
	      	<form method ="post" id="mbhForm" action="newConfirm">
				...
				<input type="hidden" name="startTime" id="startTime" value="${broadbandDetailInfo.newProductsInfo[0].start_date}"/>
			...
			<!--选择套餐 end--> 
			<!--预约安装 start-->
			<div class="install-con mb-change yy-con hide">
				...
				<div class="wf-ait clearfix">
					<ul class="bar-list">
						<li style="height:26px;line-height:26px;">
							...
							<div class="sub-text">
								<a href="javascript:choosePayMode('pay-btn1','0','现金支付')" class="bar-btn pay-btn pay-btn1 active">现金支付</a>
	...
	<div class="fix-br">
		<div class="affix foot-box new-foot">
		    <div class="container active-fix">
		    	<div class="fl-left"><a onclick="showTips()">温馨提示</a></div>
	...
	function getChooseMbhLevelName(chooseGiftElementId,price,term){
		...
		$("#termSpan").html(term);
		$.ajax({
			  url: ctx+"newO2oMbh/newGetTvProduct",
			  ...
			  success: function(e){
				  ...
				  for(var i=0;i<e.abledChooseMbhItemList.length;i++){
					  ...
					  if(e.abledChooseMbhItemList[i].broadbandItemName == "芒果TV"){
						  ...
						  tvProductHtml += '<span class="pull-left" disabled="disabled">';
						  tvProductHtml += '<input type="radio" class="cbox" id="wl" name="form1_Mbh"  onclick="getChooseTvProduct(\''+e.abledChooseMbhItemList[i].goodsSkuId+'\',\''+e.abledChooseMbhItemList[i].productId+'\',\''+e.abledChooseMbhItemList[i].packageId+'\',\''+e.abledChooseMbhItemList[i].discntCode+'\',\''+e.abledChooseMbhItemList[i].discntCode+'\')"  value="1"/>';
						  tvProductHtml += e.abledChooseMbhItemList[i].broadbandItemName + "<br/>";
					  }
	...
	function showTips(){
		showAlert("1、魔百和年包使用到期后恢复为标准资费10元/月，直接从用户手机账户收取。用户也可选择退订电视服务，或者续订礼包产品。<br/>2、续订魔百和电视将在次月生效。<br/>3、对于新装、续费裸宽用户可办理魔百和电视业务，魔百和电视费用将于手机合并捆绑后付费。<br/>4、若前期已办理魔百和电视且正处于按月收费状态，则办理魔百和年包后原魔百和计费规则取消，次月魔百和年包生效且同时变更为魔百和年包计费规则。");
	}
